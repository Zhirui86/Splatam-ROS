#!/home/agilex/anaconda3/envs/Gaussians4D/bin/python
...
from SplaTAM.datasets.gradslam_datasets import (
    load_dataset_config,
    ICLDataset,
    ReplicaDataset,
    ReplicaV2Dataset,
...
def initialize_first_timestep(config, color, depth, intrinsics, pose, num_frames, scene_radius_depth_ratio, mean_sq_dist_method, densify_dataset=None):
    # Get RGB-D Data & Camera Parameters

    # Process RGB-D Data

    resize = T.Resize((config["data"]["desired_image_height"], config["data"]["desired_image_width"]))
    color = resize(color) / 255
    depth = resize(depth)
...
def rgbd_slam(config: dict, rgb_tensor, depth_tensor):
    global time_idx, num_frames, params, variables, intrinsics, first_frame_w2c, cam, keyframe_list, keyframe_time_indices
    global gt_w2c_all_frames, tracking_iter_time_sum, tracking_iter_time_count, mapping_iter_time_sum
    global mapping_iter_time_count, tracking_frame_time_sum, tracking_frame_time_count
    global mapping_frame_time_sum, mapping_frame_time_count
    ...
    if time_idx == 0 :
        ...
        #     )
            ...
            #     densify_intrinsics, densify_cam = initialize_first_timestep(rgb_tensor, depth_tensor, num_frames,
            #                                                                 config['scene_radius_depth_ratio'],
            #                                                                 config['mean_sq_dist_method'],
            #                                                                 )                                                                                                                  
        
        # Initialize Parameters & Canoncial Camera parameters
            pose_init = torch.eye(4).float()
            intrinsics = torch.tensor([
        [607.3651733398438, 0, 319.8575134277344],
        [0, 607.43896484375, 252.25210571289062],
        [0, 0, 1]
            ]).float().to(device)

            params, variables, intrinsics, first_frame_w2c, cam = initialize_first_timestep(config, rgb_tensor, depth_tensor, intrinsics, pose_init, num_frames, 
                                                                                            config['scene_radius_depth_ratio'],
                                                                                            config['mean_sq_dist_method'])
    ...
    if config['load_checkpoint']:
        ...
        for iter_time_idx in range(checkpoint_time_idx):
            # Load RGBD frames incrementally instead of all frames
            color, depth = rgb_tensor, depth_tensor
            ...
            if iter_time_idx in keyframe_time_indices:
                ...
                resize = T.Resize(config["data"]["desired_img_height"], config["data"]["desired_img_width"])
                color = resize(color) / 255
                depth = resize(depth)
                curr_keyframe = {'id': iter_time_idx, 'est_w2c': curr_w2c, 'color': color, 'depth': depth}
    ...
    curr_data = {'cam': cam, 'im': color, 'depth': depth, 'id': time_idx, 'intrinsics': intrinsics, 
                    'w2c': first_frame_w2c }
...
def depth_callback(msg):
    global depth_queue
    if depth_queue.qsize() < 300:
        depth_queue.put(msg)
    else:
        pass    

# def timer_callback(event):
    # global latest_rgb_img, latest_depth_img, pro_rgb_pub, pro_depth_pub
    # if latest_rgb_img is not None and latest_depth_img is not None:
       
    #     # pro_rgb_pub.publish(latest_rgb_img)
    #     # pro_depth_pub.publish(latest_depth_img)
       
    #     # pro_rgb_sub = message_filters.Subscriber("/processed_rgb_image", Image)
    #     # pro_depth_sub = message_filters.Subscriber("/processed_depth_image", Image)
    

    #     time_diff = abs(latest_rgb_img.header.stamp.to_sec() - latest_depth_img.header.stamp.to_sec())
    #     if time_diff < 0.04:
    #         process_sensors(latest_rgb_img, latest_depth_img)
            
    #         latest_rgb_img = None
    #         latest_depth_img = None


def process_sensors(rgb_img, depth_img):
    global experiment
    global time_idx
    # global image_pub  
    ...
    try:
        cv_rgb = bridge.imgmsg_to_cv2(rgb_img, "bgr8").astype(np.float32)
        cv_depth = bridge.imgmsg_to_cv2(depth_img).astype(np.float32)

        rgb_tensor = torch.from_numpy(cv_rgb).permute(2, 0, 1).float().to("cuda") 
        depth_tensor = torch.from_numpy(cv_depth).unsqueeze(0).float().to("cuda")

        depth_tensor = depth_tensor * 0.001
        if time_idx <= 299:
            rgbd_slam(experiment.config, rgb_tensor, depth_tensor)
            time_idx += 1

        # if image_pub is not None:
            
        #     try:
        #         rospy.loginfo("Publishing image to topic")
        #         image_pub.publish(rgb_img)
        ...
        print("图像转换错误：", CvBridgeError) 
    

def save_rgb_image(cv_rgb):
    save_dir = "small_car/small_car/pngs" 
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    image_filename = os.path.join(save_dir, f"rgb_{time_idx}.png")  
    cv2.imwrite(image_filename, cv_rgb)
    rospy.loginfo(f"Saved RGB image as PNG to {image_filename}")


def main_pro():
    global rgb_queue, depth_queue
    for _ in range(300):
        try:
            # 从 RGB 队列中取出图像消息
            rgb_msg = rgb_queue.get(timeout=1.0)
            
            # 从深度队列中取出图像消息
            depth_msg = depth_queue.get(timeout=1.0)

            time_diff = abs(rgb_msg.header.stamp.to_sec() - depth_msg.header.stamp.to_sec())
            
            if time_diff < 0.05:
                process_sensors(rgb_msg, depth_msg)
            else:
                pass
...
if __name__ == "__main__":
    ...
    # ros init
    rospy.init_node("Gaussians_demo")
    ...
    # image_pub = rospy.Publisher("/processed_rgb_image", Image, queue_size=1)
    # pro_rgb_pub = rospy.Publisher("/processed_rgb_image", Image, queue_size=1)
    # pro_depth_pub = rospy.Publisher("/processed_depth_image", Image, queue_size=1)
    ...
    main_pro()
    # 停止继续往队列中添加图像
    # rospy.loginfo("Stopping image input to queues.")
    # rospy.sleep(500.0)  # 等待确保图像处理线程能够处理完队列中的图像

    # 启动图像处理线程
    # image_processing_thread = Thread(target=main_pro)
    ...
    # 两个订阅话题同步
    # ts = message_filters.ApproximateTimeSynchronizer([rgb_sub, depth_sub], queue_size=10 , slop=0.1, allow_headerless=True)
    # ts.registerCallback(process_sensors)
